...
3. **`GROUP BY`** – SQL builts the column list given for grouping. See **[`GROUP BY`](#group-by)**.
   - referencing columns using ordinal position or aliases:
      ...
      - example:
         ...
         ```
         where parser find whatever is refered by the position or aliases from the `SELECT` list and just copy paste it under GROUP BY
      - Other systems (e.g., SQL Server, PostgreSQL, Oracle) do not allow that;
   - `DISTINCT` inside agg functions
4. ***Aggregate functions*** (per-group): evaluated after `GROUP BY`, before output. See **[Aggregate Functions](#aggregate-functions)**.
   - Keywords inside agg_funcs
      - `DISTINCT`
      - `ORDER BY`/`SEPARTOR` for `GROUP_CONCAT()`
...
7. **`DISTINCT`**: remove duplicates after the entire SELECT list is built.
   
8. **`UNION`/`UNION ALL`** - Vertically concatenate 2 table with same number of columns.
9.  **`ORDER BY`** – Sort result; may reference aliases, aggregates, window results. See **[`ORDER BY`](#order-by-1)**.
   - order with `DISTINCT` from SELECT list
...
### `RECURSIVE` in CTEs
- You can create recursive CTEs by adding the `RECURSIVE` keyword after `WITH`. 
- anchor
...
- During recursion, the self-referenced CTE is treated like “the rows from the previous iteration” (the frontier / working set), not the entire accumulated result
- Syntax:
   ```sql
